...
export async function GET(request, context) {
  ...
  try {
    ...
    const results = await listResultsForClassMember(row.id, email);
    if (results === null) {
    ...
    console.error("GET /api/admin/classes/[id]/members/[email]/results failed", error);
    return NextResponse.json(
      { error: "Không thể tải kết quả học viên. Vui lòng thử lại." },
      { status: 500 }
    );
  }
}
