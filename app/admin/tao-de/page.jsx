...
function RowPreview({ src, alt }) {
  if (!src) {
    return (
      <div className="flex h-28 items-center justify-center rounded-lg border border-dashed border-rose-200 bg-rose-50/50 text-xs text-gray-500">
        Chưa có ảnh
      </div>
    );
  }
  return (
    <div className="relative h-28 w-full overflow-hidden rounded-lg border border-rose-100 bg-white">
      <Image
        src={src}
        alt={alt}
        fill
...
export default function TaoDePage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-rose-50 via-white to-white p-4 text-black">
      <div className="w-full space-y-6">
        <header className="rounded-2xl border border-rose-100 bg-white p-5 shadow-sm sm:p-6">
          <p className="mb-2 inline-flex rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-800">
            Thêm đề (AI / thủ công)
          </p>
          <h1 className="text-xl font-bold sm:text-2xl">Tạo đề mới từ template</h1>
          <p className="mt-2 max-w-2xl text-sm text-gray-600">
            Khung 50 câu copy từ{" "}
            <code className="rounded bg-rose-50 px-1">Listen83</code> hoặc{" "}
          ...
          <section className="rounded-3xl border border-rose-100 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="text-sm font-medium">
                Loại bài
                <select
                  className="mt-1 h-11 w-full rounded-xl border border-rose-200 bg-white px-3"
                  value={testType}
              ...
              <label className="text-sm font-medium">
                Mã đề (setKey)
                <input
                  className="mt-1 h-11 w-full rounded-xl border border-rose-200 px-3"
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              {createMsg && (
                <p className="text-sm text-gray-700">{createMsg}</p>
              )}
            </div>
          </section>
        )}
        ...
        {phase === PHASE.BULK && (
          <section className="rounded-3xl border border-rose-100 bg-white p-6 shadow-sm">
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="rounded-xl border border-rose-200 px-4 py-2 text-sm hover:bg-rose-50/50"
                >
                  Tạo đề khác
                </button>
                <button
            ...
            <div className="mb-4 rounded-2xl border border-rose-200 bg-rose-50/80 p-4">
              <p className="mb-2 text-sm font-semibold text-[#b61e3b]">
                Chọn 50 ảnh một lần
              </p>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <label className="cursor-pointer rounded-xl bg-[#e5441a] px-4 py-2.5 text-sm font-semibold text-white shadow-md hover:bg-[#cf3c15]">
                ...
                {bulkHint && (
                  <span className="max-w-xl text-sm text-gray-800">{bulkHint}</span>
                )}
              </div>
              <div
                className="mt-3 flex min-h-[88px] items-center justify-center rounded-xl border-2 border-dashed border-rose-300 bg-white/60 px-4 text-center text-sm text-gray-700"
                onDragOver={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                }}
            ...
            <div className="mb-4 rounded-2xl border border-rose-200 bg-rose-50/80 p-4">
              ...
              <textarea
                className="mb-2 min-h-[88px] w-full rounded-xl border border-rose-200 bg-white px-3 py-2 font-mono text-sm text-black placeholder:text-gray-400"
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button
                  ...
                  className="rounded-xl bg-[#e5441a] px-4 py-2 text-sm font-semibold text-white hover:bg-[#cf3c15]"
                >
                  Áp dụng đáp án
                </button>
                {answersBulkError && (
            ...
            <div className="max-h-[70vh] overflow-y-auto rounded-2xl border border-rose-100">
              <div className="sticky top-0 z-10 grid grid-cols-[minmax(0,1fr)] gap-2 border-b border-rose-100 bg-rose-50/50 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-gray-600 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                    ...
                    >
                      <div className="flex flex-col gap-2">
                        <span className="text-xs font-semibold text-gray-500">
                          Câu {r.id}{" "}
                          <span className="font-mono text-[#b61e3b]">
                            · Đáp án: {r.correctAnswer}
                          </span>
                        </span>
                        <label className="cursor-pointer rounded-lg border border-dashed border-rose-200 bg-rose-50/50 px-3 py-2 text-sm hover:bg-rose-50">
                          Chọn ảnh
                          <input
                            type="file"
                            accept="image/*"
