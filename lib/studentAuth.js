...
export function getStudentSessionSecret() {
  return (
    process.env.STUDENT_SESSION_SECRET || "tienghancohue-student-dev-secret"
  );
}
...
/** @param {string} secret */
export async function createStudentCookieValue(secret) {
  const maxAgeSec = 60 * 60 * 24 * 14;
  const exp = Math.floor(Date.now() / 1000) + maxAgeSec;
  const msg = `student:${exp}`;
...
export async function verifyStudentCookieValue(secret, cookieValue) {
...
export async function requireStudent(request) {
  ...
  if (!(await verifyStudentCookieValue(secret, token))) {
